...
echo ""
echo ""
echo "  ####################### Csa Digital cPanel Otomatik Kurulum ve Optimizasyon #######################  "
echo ""
echo ""
echo "Çıkış Yapmak için (CTRL + C to cancel)"
sleep 10
...
yum clean all
echo "####### Gerekli Dosyaların Kurulumu Başarıyla Tamamlandı ########"
...
echo "######### Özelleştirme Tamamlandı ########"
echo "######### DNS ve Ağ Yapılandırması Yapılıyor ########"
...
echo "######### DNS ve Ağ Yapılandırması Tamamlandı ########"
...
echo "####### END INSTALLING CPANEL #######"
	...
	if [ -d /usr/local/cpanel/whostmgr/docroot/cgi/configserver/csf ] ; then
			...
			if [ "x$yesno" = "xy" ] ; then
				...
				/usr/bin/wget https://raw.githubusercontent.com/csadigital/cPanel-Auto-Config/main/csf.allow -O /etc/csf/csf.allow  &>/dev/null
				/usr/bin/wget https://raw.githubusercontent.com/csadigital/cPanel-Auto-Config/main/csf.conf -O /etc/csf/csf.conf &>/dev/null
...
/usr/bin/sed -i 's/memory_limit = .*/memory_limit = 900M/' /opt/cpanel/ea-php*/root/etc/php.ini &>/dev/null
...
if [ -f /usr/share/zoneinfo/Europe/Istanbul ]; then
    echo "Türkiye'nin İstanbul şehri için zaman dilimi ayarlanıyor..."
